...
def predict_word_from_sign(index, tmp_img):
    ...
    transcription = {index:'', index+1:''}

    for i in (index, index+1):
        print('Paddding images.....')
        if i==index:
            tmp_img_pad = pad(tmp_img[:64])
        else:
            tmp_img_pad = pad(tmp_img[10:])
        print(len(tmp_img_pad))
        print('Images padded...')

        # Run through the data augmentation
        # 64 x 224 x 224 x 3
        tmp_img_pad = test_transforms(tmp_img_pad)
        ret_img = video_to_tensor(tmp_img_pad)
...
imgs = load_rgb_frames_from_video(vid_root=VIDEO_ROOT, vid="combined", start=0, num=64)
print('Images loaded')
...
while i < range_distance:
    tmp_img = imgs[i:i+74]
    string_predict.append(dask.delayed(predict_word_from_sign)(j, tmp_img))
    i+=20
    j+=2

temp_dask = dask.delayed(get_final_phrase)(j+1, string_predict)
print('ok')
